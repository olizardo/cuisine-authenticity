...
Model performance was evaluated using the Watanabe-Akaike Information Criterion (WAIC) computed across 4,000 post-warmup draws for all sixteen completed specifications in the factorial taxonomy. Table 1 and Figure 1 report the resulting information criteria, standard errors, effective parameters ($p_{\text{WAIC}}$), and comparative fit differences ($\Delta \text{WAIC}$) relative to the best-fitting model.

![](Plots/model_fit_comparison.png){fig-align="center" width="100%"}

Table 1: Consolidated WAIC Model Fit Comparison across the Full Factorial Taxonomy

| Rank | Substantive Domain | Model Specification | Threshold Constraint | Random Effects | WAIC (SE) | $p_{\text{WAIC}}$ | $\Delta \text{WAIC}$ |
|:---:|:---|:---|:---:|:---:|:---:|:---:|:---:|
| **1** | **Taste Dispositions** | **Cell 4: Relaxed RS** | Relaxed (`cs`) | Crossed Slopes | **54,348.26 (247.35)** | 1,246.1 | **0.00** |
| **2** | **Dining Practices** | **Cell 4: Relaxed RS** | Relaxed (`cs`) | Crossed Slopes | **54,446.72 (251.27)** | 1,254.4 | **+98.46** |
| **3** | **Taste Dispositions** | **Cell 2: Relaxed RI** | Relaxed (`cs`) | Random Intercepts | **54,586.18 (245.60)** | 1,187.5 | **+237.93** |
| **4** | **Cosmopolitan** | **Cell 4: Relaxed RS** | Relaxed (`cs`) | Crossed Slopes | **54,804.97 (245.00)** | 1,235.7 | **+456.72** |
| **5** | **Dining Practices** | **Cell 2: Relaxed RI** | Relaxed (`cs`) | Random Intercepts | **54,818.51 (247.60)** | 1,185.9 | **+470.25** |
| **6** | **Cultural Capital (Base)** | **Cell 4: Relaxed RS** | Relaxed (`cs`) | Crossed Slopes | **54,961.97 (244.84)** | 1,217.9 | **+613.71** |
| **7** | **Cosmopolitan** | **Cell 2: Relaxed RI** | Relaxed (`cs`) | Random Intercepts | **55,069.70 (243.43)** | 1,176.9 | **+721.44** |
| **8** | **Cultural Capital (Base)** | **Cell 3: Strict RS** | Strict Proportional Odds | Crossed Slopes | **55,171.37 (243.27)** | 1,185.2 | **+823.11** |
| **9** | **Dining Practices** | **Cell 3: Strict RS** | Strict Proportional Odds | Crossed Slopes | **55,176.81 (242.61)** | 1,160.9 | **+828.55** |
| **10** | **Cultural Capital (Base)** | **Cell 2: Relaxed RI** | Relaxed (`cs`) | Random Intercepts | **55,177.51 (243.10)** | 1,173.3 | **+829.25** |
| **11** | **Taste Dispositions** | **Cell 3: Strict RS** | Strict Proportional Odds | Crossed Slopes | **55,248.60 (242.35)** | 1,162.9 | **+900.34** |
| **12** | **Cosmopolitan** | **Cell 3: Strict RS** | Strict Proportional Odds | Crossed Slopes | **55,296.93 (242.10)** | 1,151.7 | **+948.67** |
| **13** | **Cosmopolitan** | **Cell 1: Strict RI** | Strict Proportional Odds | Random Intercepts | **55,309.75 (241.93)** | 1,141.8 | **+961.50** |
| **14** | **Cultural Capital (Base)** | **Cell 1: Strict RI** | Strict Proportional Odds | Random Intercepts | **55,310.59 (241.69)** | 1,141.6 | **+962.33** |
| **15** | **Taste Dispositions** | **Cell 1: Strict RI** | Strict Proportional Odds | Random Intercepts | **55,311.35 (242.25)** | 1,143.1 | **+963.10** |
| **16** | **Dining Practices** | **Cell 1: Strict RI** | Strict Proportional Odds | Random Intercepts | **55,312.28 (241.61)** | 1,138.4 | **+964.02** |

*Note:* "Relaxed" indicates category-specific threshold transitions (`cs()`). "Crossed Slopes" indicates crossed cuisine-level random slopes on continuous focal predictors.

The information criterion comparisons across the full sixteen specifications reveal two clear empirical patterns:
1. **The Primacy of Relaxed Category-Specific Thresholds:** Relaxing the proportional odds constraint yields decisive predictive gains across all theoretical domains. Models incorporating category-specific threshold transitions (`cs()`) consistently outperform their strict proportional odds counterparts. Taste Dispositions with relaxed transitions achieves the lowest WAIC overall ($\text{WAIC} = 54,348.26$), followed closely by Dining Practices with relaxed transitions ($\text{WAIC} = 54,446.72$).
...
Table 2 presents the cross-specification stability metrics for the fifteen culinary traditions, and Figure 2 displays the consensus posterior distributions across all estimated baseline and extension specifications.

| Culinary Tradition | Models ($k$) | Baseline Intercept Range $[\min, \max]$ | 95% Credible Envelope | Directional Consensus Classification |
|:---|:---:|:---:|:---:|:---|
| **French** | 7 | $[+0.676, +0.689]$ | $[+0.524, +0.850]$ | **Credibly Pro-Chef ($P > 0 = 100\%$)** |
| **Japanese** | 7 | $[+0.280, +0.290]$ | $[+0.148, +0.446]$ | **Credibly Pro-Chef ($P > 0 = 100\%$)** |
| **Swedish** | 7 | $[+0.191, +0.200]$ | $[+0.063, +0.355]$ | **Credibly Pro-Chef ($P > 0 \ge 99.9\%$)** |
| **Korean** | 7 | $[+0.073, +0.083]$ | $[-0.052, +0.237]$ | **Spans Zero** |
| **Italian** | 7 | $[+0.071, +0.080]$ | $[-0.057, +0.232]$ | **Spans Zero** |
| **Vietnamese** | 7 | $[-0.047, -0.036]$ | $[-0.204, +0.091]$ | **Spans Zero** |
| **Peruvian** | 7 | $[-0.062, -0.052]$ | $[-0.222, +0.073]$ | **Spans Zero** |
| **Moroccan** | 7 | $[-0.080, -0.070]$ | $[-0.239, +0.055]$ | **Spans Zero** |
| **Lebanese** | 7 | $[-0.081, -0.071]$ | $[-0.238, +0.056]$ | **Spans Zero** |
| **Pakistani** | 7 | $[-0.142, -0.134]$ | $[-0.303, -0.005]$ | **Credibly Domestic Elder ($P < 0 \ge 98.8\%$)** |
| **Mexican** | 7 | $[-0.147, -0.137]$ | $[-0.304, -0.006]$ | **Credibly Domestic Elder ($P < 0 \ge 99.0\%$)** |
| **Ethiopian** | 7 | $[-0.169, -0.157]$ | $[-0.326, -0.021]$ | **Credibly Domestic Elder ($P < 0 \ge 99.8\%$)** |
| **Jamaican** | 7 | $[-0.223, -0.214]$ | $[-0.383, -0.082]$ | **Credibly Domestic Elder ($P < 0 = 100\%$)** |
| **Nigerian** | 7 | $[-0.248, -0.238]$ | $[-0.409, -0.103]$ | **Credibly Domestic Elder ($P < 0 = 100\%$)** |
| **Native American** | 7 | $[-0.260, -0.250]$ | $[-0.420, -0.118]$ | **Credibly Domestic Elder ($P < 0 = 100\%$)** |

![](Plots/cuisine_random_effects.png){fig-align="center" width="100%"}

Across all seven baseline and extension specifications, the culinary hierarchy exhibits exceptional structural stability:
1. **Consecrated Haute Tiers:** French cuisine occupies an isolated position at the extreme professionalization pole ($\text{medians } \in [+0.676, +0.689]$, $P > 0 = 100\%$), followed by Japanese ($\text{medians } \in [+0.280, +0.290]$, $P > 0 = 100\%$) and Swedish traditions ($\text{medians } \in [+0.191, +0.200]$, $P > 0 \ge 99.9\%$). Across all model configurations, these cuisines are decisively understood as achieving authentic excellence through elite restaurant execution.
2. **Intermediate Traditions:** Italian, Korean, Vietnamese, Peruvian, Moroccan, and Lebanese cuisines occupy an intermediate middle zone where baseline intercepts tightly center near zero, indicating broad public consensus around a balance between home and restaurant excellence.
3. **Subaltern and Heritage Traditions:** A robust majority of non-Western and marginalized traditions are credibly anchored in domestic elder authenticity across all models: Native American ($\text{medians } \in [-0.260, -0.250]$), Nigerian ($\text{medians } \in [-0.248, -0.238]$), Jamaican ($\text{medians } \in [-0.223, -0.214]$), Ethiopian ($\text{medians } \in [-0.169, -0.157]$), Mexican ($\text{medians } \in [-0.147, -0.137]$), and Pakistani traditions ($\text{medians } \in [-0.142, -0.134]$), with $100\%$ directional consensus across specifications.
...
Rather than conditioning substantive conclusions on a single model choice, the **cross-specification consensus framework** tracks the posterior distribution of each focal parameter across all $k$ estimated models where the variable appears. For each predictor, we compute:
...
Table 2 presents the cross-specification stability metrics across all estimated models.

| Substantive Domain | Predictor Variable | Models ($k$) | Posterior Median Range $[\min, \max]$ | 95% Credible Envelope | Consensus Classification |
|:---|:---|:---:|:---:|:---:|:---|
| **Political Ideology** | Social Conservatism | 7 | $[+0.109, +0.193]$ | $[+0.021, +0.313]$ | **Credibly Positive (Pro-Chef, 100%)** |
| | Economic Conservatism | 7 | $[-0.043, +0.000]$ | $[-0.151, +0.063]$ | **Consistently Spans Zero** |
| **Cultural Capital** | Educational Attainment | 7 | $[+0.053, +0.122]$ | $[-0.016, +0.191]$ | **Credibly Positive (Pro-Chef)** |
| | Childhood Arts Socialization | 7 | $[-0.094, -0.025]$ | $[-0.155, +0.049]$ | **Credibly Domestic Elder (with Adult Controls)** |
| | Parental Education | 7 | $[-0.040, -0.019]$ | $[-0.110, +0.051]$ | **Consistently Spans Zero** |
| **Dining Practices** | Adult Highbrow Arts Attendance | 1 | $+0.141$ | $[+0.069, +0.213]$ | **Credibly Positive (Pro-Chef, 100%)** |
| | Fine Dining Frequency | 1 | $+0.090$ | $[+0.005, +0.177]$ | **Credibly Positive (Pro-Chef, 98%)** |
| | Fast Food Frequency | 1 | $+0.029$ | $[-0.031, +0.088]$ | **Spans Zero** |
| **Taste Dispositions** | Dispositions: Exotic / Authentic | 1 | $-0.085$ | $[-0.148, -0.022]$ | **Credibly Domestic Elder (99.4%)** |
| | Dispositions: Light / Fresh | 1 | $-0.045$ | $[-0.105, +0.017]$ | **Spans Zero** |
| | Dispositions: Rich / Hearty | 1 | $-0.045$ | $[-0.107, +0.018]$ | **Spans Zero** |
| | Dispositions: Familiar Comfort | 1 | $-0.005$ | $[-0.063, +0.053]$ | **Spans Zero** |
| **Cosmopolitan Capital** | Friendship Network Diversity | 1 | $+0.061$ | $[+0.002, +0.121]$ | **Credibly Positive (Pro-Chef, 97.8%)** |
| | Global Citizen Identity | 1 | $+0.038$ | $[-0.027, +0.103]$ | **Spans Zero** |
| **Demographics** | Gender: Woman | 7 | $[-0.178, -0.162]$ | $[-0.298, -0.054]$ | **Credibly Domestic Elder (99.9%)** |
| | Age | 7 | $[-0.064, -0.028]$ | $[-0.121, +0.031]$ | **Credibly Domestic Elder (in 5 of 7 models)** |
