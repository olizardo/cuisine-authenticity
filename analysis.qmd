...
Model performance was evaluated using the Watanabe-Akaike Information Criterion (WAIC) computed across 4,000 post-warmup draws. Table 1 and Figure 1 report the resulting information criteria, standard errors, effective parameters ($p_{\text{WAIC}}$), and comparative fit differences ordered by increasing model complexity starting with **Model 1 (Cultural Capital [Strict, RI])** as the reference baseline.

![](Plots/model_fit_comparison.png){fig-align="center" width="100%"}

| Domain | Model Specification | Threshold Constraint | Random Effects | WAIC (SE) | $p_{\text{WAIC}}$ | $\Delta \text{WAIC}_{\text{vs. M1}}$ | Relative Fit vs. Model 1 |
|:---|:---|:---:|:---:|:---:|:---:|:---:|:---|
| **Core Baseline** | **Model 1: Strict RI** | Strict Proportional Odds | Random Intercepts | 55,310.59 (241.69) | 1,141.6 | **0.00** | *Reference Baseline* |
...
| **Dining Practices** | **Practices Strict RI** | Strict Proportional Odds | Random Intercepts | 55,312.19 (241.87) | 1,148.9 | **+1.60** | Indistinguishable from M1 |
| | **Practices Relaxed RI** | Relaxed (`cs`) | Random Intercepts | **54,818.89 (248.11)** | 1,237.9 | **-491.70** | Decisive Improvement |
...
| **Taste Dispositions** | **Dispositions Strict RI** | Strict Proportional Odds | Random Intercepts | 55,310.82 (241.77) | 1,146.7 | **+0.23** | Indistinguishable from M1 |
| | **Dispositions Relaxed RI** | Relaxed (`cs`) | Random Intercepts | **54,585.83 (246.04)** | 1,223.1 | **-724.76** | Decisive Improvement (Top Model) |
...
| **Cosmopolitan Capital** | **Cosmopolitan Relaxed RI** | Relaxed (`cs`) | Random Intercepts | **55,069.96 (243.34)** | 1,189.6 | **-240.63** | Decisive Improvement |
| | **Cosmopolitan Strict RS** | Strict Proportional Odds | Crossed Random Slopes | 55,296.93 (242.10) | 1,151.7 | **-13.66** | Modest Improvement |

*Note:* "Relaxed" indicates category-specific threshold transitions (`cs()`). "Random Slopes" indicates crossed cuisine-level random slopes on continuous focal predictors.

The information criterion comparisons across the completed twelve specifications reveal clear patterns:
1. **The Primacy of Relaxed Category-Specific Thresholds:** Relaxing the proportional odds constraint yields massive predictive gains across all theoretical domains. Taste Dispositions with relaxed transitions achieves the lowest WAIC overall, producing a **724.76-point improvement** over the baseline strict model, followed by Dining Practices with relaxed transitions ($\Delta \text{WAIC} = -491.70$).
2. **Structural Slopes vs. Threshold Flexibility:** When restricted to strict proportional odds, adding cuisine random slopes yields moderate gains ($\Delta \text{WAIC} \approx -133$ to $-139$), but category-specific cutpoint flexibility captures substantially more empirical structure across all response tiers.
...
Table 1 presents the cross-specification stability metrics for the fifteen culinary traditions, and Figure 2 displays the consensus posterior distributions across all estimated baseline and extension specifications.
